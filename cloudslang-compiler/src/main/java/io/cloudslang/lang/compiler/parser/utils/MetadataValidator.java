...
public class MetadataValidator {
    ...
    public void validate(SlangSource source) {
        ...
        try (BufferedReader reader = new BufferedReader(new StringReader(source.getContent()))) {
            ...
            while (line != null) {
                ...
                validateNewlineBetweenDifferentTags(previousTagLine, previousMetadataLine, line);
            }
            validateHashTagLines(lineBeforeBlockStartTag, lineAfterBlockEndTag);
        } catch (IOException e) {
    ...
    private void validateNewlineBetweenDifferentTags(String previousTagLine, String previousMetadataLine, String line) {
        if (previousTagLine != null && bothLinesContainTags(previousTagLine, line) &&
            ...
            throw new RuntimeException("Newline with metadata prefix missing '#!' in the description before the " +
                    "following line: " + System.lineSeparator() + line);
        }
    }
