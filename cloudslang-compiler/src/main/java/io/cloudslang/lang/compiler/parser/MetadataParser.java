...
public class MetadataParser {
    ...
    private ParsedDescriptionData processRawLines(List<String> lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            if (lines.get(i).startsWith(namespace)) {
                break;
            }
            if (descriptionStarted) {
                descriptionRawLines.add(lines.get(i));
            }
            if (descriptionPatternMatcher.matchesDescriptionStart(lines.get(i))) {
                descriptionStarted = true;
                descriptionStartLine = i;
                handleBlockStart(descriptionBuilder, i);
            }
            if (descriptionPatternMatcher.matchesDescriptionEnd(lines.get(i))) {
                if (descriptionStarted) {
                    descriptionEndLine = i;
                    break;
                } else {
