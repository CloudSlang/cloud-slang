...
public class StepExecutionData extends AbstractExecutionData {
    ...
    @SuppressWarnings("unused")
    public void beginExternalStep(@Param(ScoreLangConstants.STEP_INPUTS_KEY) List<Argument> stepInputs,
                          @Param(ScoreLangConstants.LOOP_KEY) LoopStatement loop,
                          @Param(ScoreLangConstants.RUN_ENV) RunEnvironment runEnv,
                          @Param(EXECUTION_RUNTIME_SERVICES) ExecutionRuntimeServices executionRuntimeServices,
                          @Param(ScoreLangConstants.NODE_NAME_KEY) String nodeName,
        ...
        try {
            ...
            fireEvent(
                    ...
                    runEnv,
                    ScoreLangConstants.EVENT_STEP_START,
                    "beginExternalStep execution step started",
                    LanguageEventData.StepType.STEP,
                    nodeName,
                    flowVariables
            );
            ...
            sendStartBindingArgumentsEvent(
                    ...
                    runEnv,
                    executionRuntimeServices,
                    "Pre argument binding for external step",
                    nodeName,
                    flowVariables
            );
            ...
            logger.error("There was an error running the beginExternalStep execution step of: \'" + nodeName +
                    "\'. Error is: " + e.getMessage());
            throw new RuntimeException("Error running: " + nodeName + ": " + e.getMessage(), e);
        }
    }
    ...
    @SuppressWarnings("unused")
    public void endExternalStep(@Param(ScoreLangConstants.RUN_ENV) RunEnvironment runEnv,
                        @Param(ScoreLangConstants.STEP_PUBLISH_KEY) List<Output> stepPublishValues,
                        @Param(ScoreLangConstants.STEP_NAVIGATION_KEY)
                                Map<String, ResultNavigation> stepNavigationValues,
                        @Param(EXECUTION_RUNTIME_SERVICES) ExecutionRuntimeServices executionRuntimeServices,
        ...
        try {
            ...
            logger.error("There was an error running the endExternalStep execution step of: \'" + nodeName +
                    "\'. Error is: " + e.getMessage());
            throw new RuntimeException("Error running: \'" + nodeName + "\': " + e.getMessage(), e);
        }
    }
